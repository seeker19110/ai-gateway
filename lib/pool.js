...
class AccountPool {
   ...
   */
  candidates(preferred = null, now = Date.now(), { requireTools = false, requireImages = false } = {}) {
    let usable = this.list().filter((account) => account.isAvailable(now));
    // Request có `tools`: loại thẳng những nhà cung cấp chưa nói được function calling
    // thay vì ghim vào rồi để họ lặng lẽ bỏ qua tool — client tưởng model đã cân nhắc dùng
    // hàm và không hề, hoặc tệ hơn là 400 làm đứng cả pool.
    if (requireTools) usable = usable.filter((account) => this.providerOf(account).supportsTools);
    // Cùng lý do cho ảnh: Cohere (bản gateway đang nói) không có chỗ cho `image_url` trong
    // content, nên loại trước khi thử thay vì để nó nhận một request mà nó không đọc được.
    if (requireImages) usable = usable.filter((account) => this.providerOf(account).supportsImages);
