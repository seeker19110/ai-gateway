...
class SmartRouter {
  ...
  async chat(messages, { apiKeys = {}, preferred = null, params = {}, model = null } = {}) {
    ...
    const candidates = this.candidates(preferred, now, { requireTools });
    if (!candidates.length) throw this._exhausted(now, requireTools);
  ...
  _exhausted(now, requireTools = false) {
