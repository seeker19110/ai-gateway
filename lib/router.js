...
const { hasImages } = require('./messages');
...
class SmartRouter {
  ...
  candidates(preferred = null, now = Date.now(), options = {}) {
    const all = this.pool.candidates(preferred, now, options);
  ...
  async chat(messages, { apiKeys = {}, preferred = null, params = {}, model = null } = {}) {
    ...
    const now = Date.now();
    const requireTools = Boolean(params.tools && params.tools.length);
    const requireImages = hasImages(messages);
    const candidates = this.candidates(preferred, now, { requireTools, requireImages });
    if (!candidates.length) throw this._exhausted(now, { requireTools, requireImages });
    ...
    for (const [i, account] of candidates.entries()) {
      ...
      try {
        ...
        const { text, usage, rateLimit, toolCalls } = await provider.chat(messages, account.apiKey, {
        ...
        return { text, provider: provider.name, account: account.label, usage, attempts, toolCalls };
      } catch (error) {
        lastError = error;
  ...
  _exhausted(now, { requireTools = false, requireImages = false } = {}) {
    ...
    if (requireTools) {
      const withoutTools = this.candidates(null, now, { requireTools: false });
      if (withoutTools.length) {
        return new UpstreamError(
          'Không có nhà cung cấp nào (còn tài khoản dùng được) hỗ trợ `tools` — thử OpenAI, Groq, OpenRouter, Mistral, Cerebras, DeepSeek, Together, hoặc Claude.',
          400
        );
      }
    }
    if (requireImages) {
      const withoutImages = this.candidates(null, now, { requireImages: false });
      if (withoutImages.length) {
        return new UpstreamError(
          'Không có nhà cung cấp nào (còn tài khoản dùng được) hỗ trợ ảnh (`image_url`) — thử OpenAI, Groq, OpenRouter, Mistral, Cerebras, DeepSeek, Together, Claude, hoặc Gemini.',
          400
        );
      }
    }
