...
function createApp({
  ...
  // `GATEWAY_API_KEY` chỉ chặn được truy cập TRÁI PHÉP; nó không chặn một client HỢP LỆ gọi
  // dồn dập (retry hỏng, script quên giới hạn riêng). `GATEWAY_RATE_LIMIT_RPM` đặt trần số
  // request/phút cho mỗi client — khác với cửa sổ RPM đã có sẵn cho từng TÀI KHOẢN upstream.
  const rateLimitPerMinute = Number(env.GATEWAY_RATE_LIMIT_RPM) || 0;
  if (rateLimitPerMinute > 0) {
    const rateLimiter = createRateLimiter({ limitPerMinute: rateLimitPerMinute });
    app.use(['/api/chat', '/v1/chat/completions', '/mcp'], requireWithinRateLimit(rateLimiter, rateLimitPerMinute));
 ...
 */
/** `Authorization: Bearer <x>` hoặc header `x` thô — dùng chung cho client key và rate limit. */
...
/**
 * Đếm theo API key đã xác thực nếu có — client hợp lệ nào cũng có một định danh ổn định,
 * bất kể IP thay đổi thế nào (di động, NAT chung của một văn phòng). Không có key thì đếm
 * theo IP: chưa lý tưởng (nhiều client sau một NAT dùng chung một bucket) nhưng còn hơn
 * không giới hạn gì khi `GATEWAY_API_KEY` chưa bật.
 */
function requireWithinRateLimit(limiter, limitPerMinute) {
  return (req, res, next) => {
    const identity = extractApiKey(req) || req.ip;
    const result = limiter.check(identity);
    if (!result.allowed) {
      res.set('Retry-After', String(result.retryAfterSeconds));
      return res.status(429).json({
        error: {
          message: `Vượt giới hạn ${limitPerMinute} request/phút (GATEWAY_RATE_LIMIT_RPM), thử lại sau ${result.retryAfterSeconds}s`,
          status: 429
        }
      });
    }
