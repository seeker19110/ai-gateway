...
function createApp({
  ...
  // `/api/providers/*` đọc/xóa cooldown và thử API key; `/api/claude/oauth/*` mở/đóng một
  // phiên đăng nhập subscription — cả hai không nên mở cho bất kỳ ai gọi tới cổng, khác với
  // `/api/chat` và `/v1/chat/completions` vốn là mặt hàng chính của gateway. Xác thực chỉ
  // BẬT khi có `GATEWAY_ADMIN_TOKEN`: mặc định giữ nguyên hành vi cũ (không token nào phải
  // cấu hình để chạy `npm start` lần đầu), người vận hành tự quyết định lúc nào cần khóa lại.
  if (!env.GATEWAY_ADMIN_TOKEN) {
    logger.warn('GATEWAY_ADMIN_TOKEN chưa đặt: /api/providers/* và /api/claude/oauth/* đang mở cho bất kỳ ai gọi tới cổng');
  }
  app.use(['/api/providers', '/api/claude/oauth'], requireAdminToken(env));
...
function requireAdminToken(env) {
  ...
  return (req, res, next) => {
    ...
    const match = providedBuf.length === tokenBuf.length && crypto.timingSafeEqual(providedBuf, tokenBuf);
    if (!match) {
