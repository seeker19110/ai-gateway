...
function discoverKeys(providerName, { env = process.env, apiKeys = {} } = {}) {
  ...
  // Tài khoản subscription (Claude Pro/Max): ưu tiên đọc token CLI đã đăng nhập sẵn
  // (`claude login`); máy không có CLI thì rơi về token mà gateway tự đăng nhập được qua
  // `/api/claude/oauth/*` (xem `lib/claudeOAuth.js`). Chỉ lấy MỘT trong hai — có CLI thật rồi
  // thì không cần tài khoản gateway tự quản nữa, tránh cùng một tài khoản bị đếm hai lần.
  if (providerName === 'claude') {
    const cli = discoverClaudeCliCredential(env) || discoverGatewayClaudeCredential(env);
...
/**
 * `discoverKeys` chạy lại ở MỖI request chat (`router.chat`/`streamChat` gọi
 * `configureProviders()` mỗi lượt) — đọc file đĩa đồng bộ ở đó nghĩa là MỌI request, kể cả
 * request dùng Gemini/Groq/…, đều chặn event loop bằng một `readFileSync` cho Claude. Cache
 * ngắn hạn (vài giây) giữ đúng tính "gần như tức thời" của việc phát hiện đăng nhập mới,
 * mà không biến mỗi request thành một lượt I/O.
 */
const FILE_CACHE_TTL_MS = 3000;
const fileCredentialCache = new Map(); // path -> { expiresAt, value }

function readCachedCredentialFile(file, now = Date.now()) {
  const cached = fileCredentialCache.get(file);
  ...
  try {
    ...
    value = null; // Thiếu file cũng được cache: tránh dội `readFileSync` thất bại liên tục.
  }
  fileCredentialCache.set(file, { expiresAt: now + FILE_CACHE_TTL_MS, value });
  return value;
}
...
function discoverClaudeCliCredential(env = process.env) {
  ...
  const raw = readCachedCredentialFile(file);
  if (!raw) return null; // Chưa `claude login`, hoặc chạy trên máy/CI không có CLI — im lặng bỏ qua.
...
/**
 * Token subscription mà chính GATEWAY đăng nhập được (máy không có Claude Code CLI), qua
 * `/api/claude/oauth/*`. Đọc lười (yêu cầu vòng tròn: `claudeOAuth` không đụng tới file này,
 * chỉ `accounts.js` mới cần biết cả hai nguồn) để tránh require vòng lặp lúc module nạp.
 */
function discoverGatewayClaudeCredential(env = process.env) {
  const { loadCredential, isExpired } = require('./claudeOAuth');
  const credential = loadCredential(env);
  if (!credential || isExpired(credential)) return null;
  return { label: 'claude-subscription', key: credential.accessToken };
}
