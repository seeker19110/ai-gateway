...
function createMcpRouter(router, { logger } = {}) {
  ...
  // sessionId -> { createdAt }. Sống theo tiến trình: một phiên không cần bền qua restart,
  ...
  app.post('/', async (req, res) => {
    ...
    if (!isInitialize) {
      ...
      // Có header nhưng phiên không tồn tại (server restart, hoặc phiên đã bị đóng bằng
      // DELETE): 404 để client tự tạo phiên mới, đúng hành vi spec yêu cầu.
      if (!sessions.has(sessionId)) return res.status(404).json(rpcError(body.id, -32001, 'Phiên không tồn tại hoặc đã hết hạn'));
    }
